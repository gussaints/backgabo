...
export class LocalProxyService {
  ...
  private async testConnection(): Promise<void> {
    const port = this.configService.get<number>('mongo.local_microservice');
    ...
    try {
      ...
      console.log(`Connected to LOCAL${port == 3027 ? "_DEV":"_PROD"} `);
  ...
  //                          GET BY ID
  ///////////////////////////////////////////////////////////////////////////////////
  async populatelocalProxy(id: string, param: string, microname: string): Promise<any> {
    const response: any = await lastValueFrom(
        ClientProxyFactory.create({ 
        transport: Transport.TCP, 
            options: { 
                host: "localhost", 
                port: this.configService.get('mongo.local_microservice') 
            } 
        })
